...
class TodoApp(ft.UserControl):
    def build(self):
        self.new_task = ft.TextField(
            hint_text="Напиши, что нужно не забыть сделать", on_submit=self.add_clicked, expand=True
        )
        ...
        self.items_left = ft.Text("0 задач завершено")
    ...
    async def update_async(self):
        ...
        for task in self.tasks.controls:
            ...
            if not task.completed:
                count += 1
        self.items_left.value = f"{count} задач"
...
ft.app(target=main, view=ft.WEB_BROWSER, port=6060)